...
;                  - If the script already has WM_NOTIFY, WM_MOUSEMOVE or WM_LBUTTONUP handlers then only set unregistered
...
Global $aGLVEx_Data[1][17] = [[0, 0, -1, "", -1, -1, -1, -1]]
...
;                            [n][14] = Edit width array
;						     [n][15] = ToolTip column range
;                            [n][16] = ToolTip display time
; Set scrollbar width
$aGLVEx_Data[0][8] = _WinAPI_GetSystemMetrics(2) ; Vertical scrollbar width
...
; _GUIListViewEx_EditItem:             Allow edit of ListView items for editing programatically
; _GUIListViewEx_ChangeItem:           Change ListView item content programatically
...
; _GUIListViewEx_ToolTipShow:          Show tooltips with item content when defined columns clicked
...
; _GUIListViewEx_ToolTipHide:  Called by Adlib to hide tooltip displayed by _GUIListViewEx_ToolTipShow
; ====================================================================================================================
...
Func _GUIListViewEx_ReturnArray($iIndex, $iCheck = 0)
	...
	Local $aLVArray = $aGLVEx_Data[$iIndex][2], $iDim_1 = UBound($aLVArray, 1), $iDim_2 = UBound($aLVArray, 2)

	; Check if checkbox array
	If $iCheck Then
		If $aGLVEx_Data[$iIndex][6] Then
			Local $aCheck_Array[$iDim_1]
			For $i = 1 To $iDim_1 - 1
				$aCheck_Array[$i] = _GUICtrlListView_GetItemChecked($hLV, $i - 1)
			Next
			; Remove count element if required
			If BitAND($aGLVEx_Data[$iIndex][3], 1) = 0 Then
				; Check at least one entry in array
				If $aLVArray[0][0] = 0 Then Return SetError(2, 0, "")
	...
	If $aGLVEx_Data[$iIndex][13] Then
		...
		$aRetArray[0][0] = $aLVArray[0][0]
		For $i = 1 To $iDim_1 - 1
			For $j = 0 To $iDim_2 - 1
				$aRetArray[$i][$j] = $aLVArray[$i][$aColOrder[$j + 1]]
			Next
		Next
...
Func _GUIListViewEx_ChangeItem($iLV_Index, $iRow, $iCol, $vValue)
	...
	; Rewrite ListView
	_GUIListViewEx_ReWriteLV($hGLVEx_SrcHandle, $aData_Array, $aCheck_Array, $iLV_Index, $fCheckBox)
...
; Description ...: Defines column(s) which will display the item content in a tooltip when clicked
...
; Author ........: Melba23
; Modified ......:
; Remarks .......: Function is desigend to show ListView content if column is too narrow for data within
...
EndFunc   ;==>_GUIListViewEx_GetLVCoords
...
EndFunc   ;==>_GUIListViewEx_GetCursorWnd
...
Func _GUIListViewEx_ToolTipHide()
	...
	For $i = 4 To 7
		$aGLVEx_Data[0][$i] = -1
	Next
EndFunc   ;==>_GUIListViewEx_ToolTipHide
